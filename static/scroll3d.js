...
(function () {
  ...
  const basePos     = new Float32Array(POINT_COUNT * 3);  // rest positions
  const explVec     = new Float32Array(POINT_COUNT * 3);  // explosion direction
  ...
  for (let i = 0; i < POINT_COUNT; i++) {
    ...
    const r     = 1.4 + Math.random() * 0.9;   // 1.4–2.3 radius
    ...
    /* Explosion vector: radially outward + slight random offset */
    ...
    /* Vertical gradient: bottom=blue, mid=violet, top=cyan */
    const t = (y + 2.3) / 4.6;   // normalize y to [0,1]
    const c = t < 0.5
  ...
  /* ── Build connections (LineSegments) ── */
  const DIST_THRESH = 0.38;
  const linePairs   = [];   // [iA, iB, ...]

  for (let i = 0; i < POINT_COUNT; i++) {
    let count = 0;
    for (let j = i + 1; j < POINT_COUNT; j++) {
      if (count >= 5) break;  // max 5 connections per node
  ...
  for (let k = 0; k < lineCount; k++) {
    const a = linePairs[k * 2];
    const b = linePairs[k * 2 + 1];
    linePos[k * 6]     = basePos[a * 3];
    linePos[k * 6 + 1] = basePos[a * 3 + 1];
    linePos[k * 6 + 2] = basePos[a * 3 + 2];
    linePos[k * 6 + 3] = basePos[b * 3];
    linePos[k * 6 + 4] = basePos[b * 3 + 1];
    linePos[k * 6 + 5] = basePos[b * 3 + 2];

    /* line color = avg of two endpoint colors */
    ...
    lineColors[k * 6 + 3] = (colors[a * 3]     + colors[b * 3])     * 0.5;
    lineColors[k * 6 + 4] = (colors[a * 3 + 1] + colors[b * 3 + 1]) * 0.5;
    lineColors[k * 6 + 5] = (colors[a * 3 + 2] + colors[b * 3 + 2]) * 0.5;
  }
  ...
  /* S1 → S2: rotate + zoom in, lines brighten */
  tl.to(state, {
    rotY:    Math.PI * 1.5,
    cameraZ: 2.6,
    lineOp:  0.5,
  ...
  /* S2 → S3: explode points outward, fade lines */
  tl.to(state, {
    explosion: 1,
    cameraZ:   7.0,
    lineOp:    0.0,
  ...
  /* S3 → S4: regroup + full spin, lines return */
  tl.to(state, {
    explosion: 0,
    rotY:      Math.PI * 3.5,
    cameraZ:   4.0,
  ...
  function animate() {
    ...
    } else if (ptGeo.attributes.position.array !== positions ||
               positions[0] !== basePos[0]) {
      /* snap back */
      for (let i = 0; i < POINT_COUNT * 3; i++) positions[i] = basePos[i];
      ptGeo.attributes.position.needsUpdate = true;
    }

    /* Lines stay at base positions (fade out during explosion) */
    lineMat.opacity = state.lineOp;
    ptMat.opacity   = state.pointOp;
